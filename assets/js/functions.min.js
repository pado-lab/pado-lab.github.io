    ...
    $(".header__menutoggle__item").click(function () {
      $("#menu").hasClass("visible")
        ? ($("#menu").removeClass("visible").addClass("hidden"),
          ...
          $(".header__menutoggle__lang").hasClass("ko")
...
function dominantColors() {
  if ($("#mapping")[0]) {
    var e = $(".mapping__item").map(function () {
        return $(this).attr("data-dominantcolor-hex");
      }),
      t = Math.floor(Math.random() * e.length);
    mappingRandomDominantColor = e[t];
    var o = tinycolor(mappingRandomDominantColor).getBrightness();
    if (tinycolor(mappingRandomDominantColor).isLight())
      if (
        ...
        var i = tinycolor(mappingRandomDominantColor).lighten(8).toString();
        $("#menu, #menu__header").css("background-color", i);
      } else {
        i = tinycolor(mappingRandomDominantColor).lighten(10).toString();
        $("#menu, #menu__header").css("background-color", i);
      }
    else {
      if (o < 10)
        var a = tinycolor(mappingRandomDominantColor)
          .brighten(10)
          .lighten(20)
          .toString();
      else if (o < 40)
        a = tinycolor(mappingRandomDominantColor)
          .brighten(10)
          .lighten(10)
          .toString();
      else a = tinycolor(mappingRandomDominantColor).lighten(20).toString();
  ...
  $(document).on(
    ...
    $(document).on("mouseleave", ".mapping__item .item__wrapper", function () {
      $(".projecttitle__number").text(""),
        ...
        $("#mapping").hasClass("projects--filtered")
          ? ($(".projecttitle__number").text(projectTitleNumber),
            $(".projecttitle__name").text(projectTitleName))
          : $(".mapping__item")
              .not(sameProjectNumber)
...
function mappingImgClick() {
  ...
  $(document).on("click", ".item__link__wrapper", function (e) {
    ...
    $(document).on("click", function (e) {
      var t = $(e.target);
      t.closest(".mapping__item .item__link__wrapper").length ||
...
function projectSlideshowSlick() {
  ...
  $(".slideshow__item")
    ...
    i.on({
      mouseenter: function () {
        $(".tooltip").fadeIn().html("→"), console.log("Tooltip Test Next");
      },
      touchstart: function () {
    ...
    a.on({
      mouseenter: function () {
        $(".tooltip").fadeIn().html("←"), console.log("Tooltip Test Prev");
      },
      touchstart: function () {
        $(".tooltip").html("").hide().css("visibility", "hidden");
      },
